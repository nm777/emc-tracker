...
5. Set a start date — the day wood is delivered to the site (data will be collected from that date forward)
6. Set an end date — the day air conditioning is installed and running (optional — if set, collection stops at that date; if left blank, collection continues up to today)
7. Click **Save**, then **Collect Data**
...
| `cities.json` | Project configuration (name, address, coordinates, start/end dates) — generated by the UI |
